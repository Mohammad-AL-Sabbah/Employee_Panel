...
const SupportTickets = () => {
  ...
  return (
    <div className="p-8 w-full max-w-[1600px] mx-auto bg-[#f8fafc] min-h-screen animate-in fade-in duration-700" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          ...
          <div key={i} className={`p-5 rounded-[2rem] border border-white shadow-sm flex items-center gap-4 ${stat.bg} hover:scale-105 transition-transform duration-300`}>
      ...
      <div className="bg-white/70 backdrop-blur-md p-4 rounded-[2rem] border border-white shadow-sm mb-6 flex flex-wrap items-center gap-4">
        <div className="relative flex-1 min-w-[300px]">
          <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
          <input 
            type="text" placeholder="ابحث عن تذكرة عبر الموضوع أو المعرف..." 
            value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-white border border-slate-100 rounded-2xl py-3 pr-12 pl-4 text-xs font-bold focus:ring-4 focus:ring-slate-100 transition-all outline-none text-right"
          />
        </div>
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="p-6 text-[11px] font-black text-slate-400 uppercase tracking-widest">تحديد</th>
